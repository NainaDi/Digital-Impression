...
GitHub Actions uploads a deployment artifact on pushes. When you run the workflow manually from the Actions tab, it also deploys over SSH using GitHub Secrets; there is no separate `deploy_over_ssh` option to select.
...
## GitHub Actions SSH deployment option

The workflow at `.github/workflows/shopify-mcp-middleware.yml` deploys the generated middleware package to a password-based SSH server whenever you run it manually from GitHub Actions. Push-triggered runs still only validate/build the deployment artifact.

Required GitHub Actions secrets for SSH deployment:
...
`SSH_TARGET_DIR` must be the absolute server directory that maps to the middleware URL. For the new Amazon Linux Apache setup, use `SSH_TARGET_DIR=/var/www/html/downloads/mcp` and `PUBLIC_BASE_URL=http://65.2.30.134/downloads/mcp` until DNS/SSL are configured. Apache must allow `.htaccess` overrides for that directory so `/health` and `/api/...` route to `public/index.php`.
